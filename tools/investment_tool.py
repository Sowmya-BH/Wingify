...
class InvestmentTool(BaseTool):
    """
    Processes a financial document and performs investment analysis.
    """
    name: str = "investment_tool"
    description: str = (
        "Processes a financial document (PDF path) and runs an analysis query."
    )
    ...
    def _run(self, input_string: str) -> str:
        try:
            ...
            if len(parts) != 2:
                return "Error: Input must be 'PDF_PATH, QUERY_STRING'."

            pdf_path, query = parts

            # Step 1: Ingest document
            ingestion_tool = PDFIngestionTool()
            ingest_status = ingestion_tool._run(pdf_path)
            if "Error" in ingest_status:
                return ingest_status

            # Step 2: Simulate analysis
            logger.info(f"Running investment analysis for query: {query}")
            return (
                f"Investment Analysis Completed ✅\n"
                f"Document: {pdf_path}\n"
                f"Query: {query}\n"
                f"{ingest_status}\n"
                f"(Simulated financial analysis response)"
            )
